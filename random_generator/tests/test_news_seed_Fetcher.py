...
def test_fetch_data(mock_get, mock_response):
    ...
    data = NewsSeedFetcher._NewsSeedFetcher__fetch_data()
    assert data.articles[0].title == "Sample Article Title"


@patch('random.random')
def test_encode_data(mock_random):
    ...
    news_data = NewsSeedFetcher.NewsAPIResponseModel(
        articles=[NewsSeedFetcher.NewsAPIResponseModel.ArticleModel(
            title="Sample Article Title")]
    )
    seed = NewsSeedFetcher._NewsSeedFetcher__encode_data(news_data)
...
@patch('random_generator.web.api.random.seed_fetchers.NewsSeedFetcher._NewsSeedFetcher__fetch_data')
@patch('random_generator.web.api.random.seed_fetchers.NewsSeedFetcher._NewsSeedFetcher__encode_data')
